...
#' Merge single-cell experiment data
#'
#' Methods to combine \code{\link[SingleCellExperiment]{SingleCellExperiment}}
#' or \code{\link{SingleCellMultiExperiment}} objects. The behaviors of merging
#' are the same as \code{\link[SeuratObject]{merge}} function from \pkg{Seurat}.
#'
...
.check_merge_by <- function(merge.by = NULL) {
  if (length(x = merge.by) == 0) {
    return(merge.by)
  }
  if (length(x = merge.by) > 1) {
...
.concat_lists_helper <- function(lists, merge.by) {
  ...
  new.list <- list()
  all.nms <- lapply(X = lists, FUN = names)
  ...
  if (merge.by == "same") {
    ...
    for (i in use.nms) {
      tmp <- lapply(X = lists, FUN = function(l) l[[i]])
